...
namespace System.Windows.Forms.ComponentModel.Com2Interop
{
    ...
    internal class Com2TypeInfoProcessor
    {
        ...
        private static Guid GetGuidForTypeInfo(UnsafeNativeMethods.ITypeInfo typeInfo, int[] versions)
        {
            ...
            try
            {
                NativeMethods.tagTYPEATTR typeAttr = Marshal.PtrToStructure<NativeMethods.tagTYPEATTR>(pTypeAttr);
                if (versions != null)
                {
        ...
        private static Type GetValueTypeFromTypeDesc(in NativeMethods.tagTYPEDESC typeDesc, UnsafeNativeMethods.ITypeInfo typeInfo, object[] typeData)
        {
            ...
            switch ((NativeMethods.tagVT)typeDesc.vt)
            {
                ...
                case NativeMethods.tagVT.VT_PTR:
                	...
                	NativeMethods.tagTYPEDESC refTypeDesc = Marshal.PtrToStructure<NativeMethods.tagTYPEDESC>(typeDesc.unionMember);
            ...
            try
            {
                ...
                if (refTypeInfo != null)
                {
                    ...
                    try
                    {
                        NativeMethods.tagTYPEATTR refTypeAttr = Marshal.PtrToStructure<NativeMethods.tagTYPEATTR>(pRefTypeAttr);
        ...
        private static void ProcessFunctions(UnsafeNativeMethods.ITypeInfo typeInfo, IDictionary propInfoList, int dispidToGet, int nameDispID, ref bool addAboutBox)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < typeAttr.cFuncs; i++)
                {
                    ...
                    try
                    {
                        NativeMethods.tagFUNCDESC funcDesc = Marshal.PtrToStructure<NativeMethods.tagFUNCDESC>(pFuncDesc);
                        ...
                        else
                        {
                            ...
                            NativeMethods.tagELEMDESC ed = Marshal.PtrToStructure<NativeMethods.tagELEMDESC>(funcDesc.lprgelemdescParam);
                            unsafe
                            {
                                typeDesc = *ed.tdesc;
                            }
            ...
            try
            {
                ...
                try
                {
                    ...
                    for (int i = 0; i < nItems; i++)
                    {
                        ...
                        try
                        {
                            NativeMethods.tagVARDESC varDesc = Marshal.PtrToStructure<NativeMethods.tagVARDESC>(pVarDesc);
